...
TfLiteStatus GetImage(tflite::ErrorReporter* error_reporter, int image_width,
  ...
  if (!g_is_camera_initialized) {
    ...
    if (!is_status_ok(init_status, "InitCamera failed")) return init_status;
    
    g_is_camera_initialized = true;
  }

  TfLiteStatus capture_status = PerformCapture(error_reporter);
  if (!is_status_ok(capture_status, "PerformCapture failed")) return capture_status;

  TfLiteStatus read_data_status = ReadData(error_reporter);
  if (!is_status_ok(decode_status, "ReadData failed")) return read_data_status;

  TfLiteStatus decode_status = DecodeAndProcessImage(
      error_reporter, image_width, image_height, image_data);
  if (!is_status_ok(decode_status, "DecodeAndProcessImage failed")) return decode_status;
